...
start_date=dt(2020, 1, 1).date()  # Default start date to get the price quotes
end_date = datetime.datetime.now().date() # Current date is the last day of price quotes.
    
def get_Data(ticker_list, start, end):
    
    df= yf.download(ticker_list,period = '1d',start = start, end = end,actions = True)
    ...
    dividend_recieved_by_assets = pd.DataFrame(dividend_recieved_by_assets.values,dividend_recieved_by_assets.index)
    dividend_recieved_by_assets.rename(columns = {0:'Dividend_Recieved'}, inplace = True)
    frames = [initial_price_of_assets, final_price_of_assets,dividend_recieved_by_assets]
    return_summary = pd.concat(frames,axis = 1, join = 'inner')
    return_summary['Cummilatuive Return %'] = 100* (return_summary['Final_Price']- return_summary['Initial_Price'])/return_summary['Initial_Price']
    return_summary['CAGR']=(pow((return_summary['Final_Price']/return_summary['Initial_Price']),(1/number_of_years))-1)*100
    return_summary['Cummilatuive Return % with Div.'] = (((return_summary['Final_Price']+return_summary['Dividend_Recieved'])/return_summary['Initial_Price'])-1)*100
    return_summary['CAGR with Div']=(pow(((return_summary['Final_Price']+return_summary['Dividend_Recieved'])/return_summary['Initial_Price']),(1/number_of_years))-1)*100
    ...
    dividend_return = return_summary['Dividend_Recieved']/return_summary['Initial_Price']
...
def random_portfolios(num_portfolios, meanReturns, covMatrix, risk_free_rate=0):
    ...
    for i in range(num_portfolios):
        ...
        portfolio_return,portfolio_std_dev = portfolioPerformance(weights,meanReturns , covMatrix)
...
def calculatedResults(meanReturns, covMatrix, riskFreeRate=0):
    ...
    maxSR_allocation = pd.DataFrame(maxSR_Portfolio['x'], index=meanReturns.index, columns=['Max_SR_Alloc'])
    maxSR_allocation.Max_SR_Alloc = [round(i*100,0) for i in maxSR_allocation.Max_SR_Alloc]
    
    # Min Volatility Portfolio
    minVol_Portfolio = minimizeVariance(meanReturns, covMatrix)
    minVol_returns, minVol_std = portfolioPerformance(minVol_Portfolio['x'], meanReturns, covMatrix)
    minVol_returns, minVol_std = round(minVol_returns*100,2), round(minVol_std*100,2)
    minVol_allocation = pd.DataFrame(minVol_Portfolio['x'], index=meanReturns.index, columns=['Min_Vol_Alloc'])
    minVol_allocation.Min_Vol_Alloc = [round(i*100,0) for i in minVol_allocation.Min_Vol_Alloc]
    
    
    # Bounded Portfolio Portfolio
    Bounded_Portfolio = Bounded_maxSR(meanReturns, covMatrix)
    Bounded_returns, Bounded_std = portfolioPerformance(Bounded_Portfolio['x'], meanReturns, covMatrix)
    Bounded_returns, Bounded_std = round(Bounded_returns*100,2), round(Bounded_std*100,2)
    Bounded_allocation = pd.DataFrame(Bounded_Portfolio['x'], index=meanReturns.index, columns=['Bounded_Alloc'])
    Bounded_allocation.Bounded_Alloc = [round(i*100,0) for i in Bounded_allocation.Bounded_Alloc]
...
def get_Index_Data(index_list, start, end):
    ...
    index_cummilative_daily_returns = ((1+index_daily_returns).cumprod() -1) *100
...
app.layout =  html.Div([
            ...
            html.P(['Select a minimum of 6 stocks from the below dropdown list. This application will create following portfolios for:',html.Br(), 
                    '1) Maximum Sharpe Ratio', html.Br(),
                    '2) Minimum Volatility Portfolio', html.Br(),  
                    '3) Maximum Sharpe ratio with capital allocation constraint where minimum of 2 percent and \
                        maxmium of 25 percent of capital could be invested in each stock.',  html.Br(), 
                    'The application will also compare the performance of portfolios created with equal weight protfilio and S&P 500 index']),
            
            html.Label(['From the Dropdown select minimum of 6 stocks for comparision:'],
                        style={'font-weight': 'bold'}),

            dcc.Dropdown(id='ticker_list',
                            options=stock_dropdown_list,
                            optionHeight=35,
                            value=['AAPL', 'GOOG', 'PEP', 'XOM'], #dropdown value selected automatically when page loads
                            disabled=False,                     #disable dropdown value selection
                            multi=True,                        #allow multiple dropdown values to be selected
                            searchable=True,                    #allow user-searching of dropdown values
                            placeholder='Please select 4 stocks...',     #gray, default text shown when no option is selected
            ...
            html.Div([dcc.Graph(id='pie_chart',figure = blank_fig(),
            ...
            dcc.Graph(id='portfolio_performance_graph',figure = blank_fig(),
                      style={'width': '90%', 'height': '80vh'}),
...
def update_graph( n_clicks,ticker_list): 
    ...
    if len(ticker_list) < 5:
        message = html.P('Please select minimum of 5 stocks from the dropdown list above', style = {'color':'red','font-size':'15px'})
        ...
        meanReturns, covMatrix, return_summary, df_adj_close,stock_daily_return,weights = get_Data(ticker_list,start_date,end_date)
        ...
        results, weights_rp = random_portfolios(num_portfolios,meanReturns,covMatrix , risk_free_rate=0)
        #Converting the results array to dataframe
        df = pd.DataFrame(results.T, columns =['STD_DEV', 'Return', 'Sharpe_Ratio'])
        
        global g_allocation
        
        maxSR_returns, maxSR_std, minVol_returns, minVol_std, efficientList,targetReturns,allocation,Bounded_returns,Bounded_std = calculatedResults(meanReturns, covMatrix, riskFreeRate =0)
    ...
    #Efficient Frontier
        ...
        s_columns = [{'name': col, 'id': col} for col in return_summary.columns]
        ...
        for stock in stock_list :
            ...
            title = dict(text = 'Daily Cummilative Return of Each Stock in Protfolio with Dividend',x= .05, y = .95 ),
        ...
        stock_cum_ret_fig = go.Figure(data=daily_cum_ret_data, layout=cum_layout)
        portfolio_type = ['Max_SR_Alloc', 'Min_Vol_Alloc', 'Bounded_Alloc', 'Equal_Weight']    
            
        index_daily_returns,index_cummilative_daily_returns = get_Index_Data(index_list,start_date, end_date)
        
        #Getting the cummilative return for different allocation
        i=-1
        for pt in portfolio_type:
            i=i+1
            stock_daily_return['PR '+pt]= np.average(stock_daily_return[stock_list],weights=weights_array[i], axis=1)
            stock_daily_return['CR '+pt] = ((1+ stock_daily_return['PR '+pt]).cumprod()-1)*100
        
        daily_cummilative_portfolio_ret = stock_daily_return[['CR Max_SR_Alloc','CR Min_Vol_Alloc','CR Bounded_Alloc','CR Equal_Weight']]
        portfolio_daily_return = stock_daily_return[['PR Max_SR_Alloc','PR Min_Vol_Alloc','PR Bounded_Alloc','PR Equal_Weight']]
        
        frames2 = [index_cummilative_daily_returns,daily_cummilative_portfolio_ret]
        ...
        pie_fig = make_subplots(rows=1, cols=3, 
                                ...
                                subplot_titles=['Maximum SR allocation', 'Minimum Volatility Allocation','Bounded Allocation'],
                                 )
        ...
        portfilio_standard_dev = portfilio_standard_dev.to_frame(name = 'Cummilative Return Std_dev in %')
        ...
        def max_drawdown(portfolio_daily_return_1):
            ...
            return drawdown.min()
        max_drawdowns = portfolio_daily_return_1.apply(max_drawdown,axis =0) *100
        ...
        last_day_return = last_day_return.to_frame(name = 'Cummilative Return in %')
        ...
        portfolio_summary_comp.rename(columns = {'index': 'Portfolio Type'},inplace = True)
        ps_columns = [{'name': col, 'id': col} for col in portfolio_summary_comp.columns]
        ps_data = portfolio_summary_comp.to_dict(orient = 'records')
        
        table_3_label = html.Label(['Portfolio Performance Summary' ],
                    style={'font-weight': 'bold', }),
